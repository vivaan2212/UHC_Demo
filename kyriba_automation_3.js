...
async function kyribaForecastAutomation() {
    ...
    try {
        ...
        console.log('Step 4: Updating Customer Receipts (AR) values...');

        // Customer Receipts (AR) values
        const customerReceiptsValues = [
            '1000000', '1200000', '900000', '1150000', '850000',
            '1300000', '900000', '1100000', '1300000', '1400000',
            '1200000', '1500000', '1600000'
        ];

        // Find the Customer Receipts row dynamically
        const customerReceiptsRow = page.locator('td:has-text("Customer Receipts (AR)")').locator('xpath=ancestor::tr');

        // Get all editable cells in that row (starting from column 3)
        const customerCells = await customerReceiptsRow.locator('td').all();

        // Start from index 2 (column 3, since index 0 is column 1)
        for (let i = 0; i < customerReceiptsValues.length; i++) {
            const cellIndex = i + 2; // Offset for column 3
            if (cellIndex < customerCells.length) {
                await customerCells[cellIndex].click();
                ...
                // Type new value
                await page.keyboard.type(customerReceiptsValues[i]);
                await page.keyboard.press('Enter');
                await page.waitForTimeout(300);

                console.log(`✓ Updated Customer Receipts cell ${i + 1}: ${customerReceiptsValues[i]}`);
            }
        }

        console.log('Step 5: Clicking Apply for Customer Receipts...');

        // Click Apply button - find it near Customer Receipts row
        await customerReceiptsRow.locator('button:has-text("Apply")').click();
        await page.waitForTimeout(2000);
        await page.waitForLoadState('networkidle');
        console.log('✓ Applied Customer Receipts changes');

        console.log('Step 6: Updating Payroll values...');

        // Payroll values (starting from column 10)
        const payrollValues = [
            '-343496.16', '-290261.44', '-336000.00',
            '-336000.00', '-336000.00', '-336000.00'
        ];

        // Find the Payroll row dynamically
        const payrollRow = page.locator('td:has-text("Payroll")').locator('xpath=ancestor::tr').first();

        // Get all cells in that row
        const payrollCells = await payrollRow.locator('td').all();

        // Start from index 9 (column 10, since index 0 is column 1)
        for (let i = 0; i < payrollValues.length; i++) {
            const cellIndex = i + 9; // Offset for column 10
            if (cellIndex < payrollCells.length) {
                await payrollCells[cellIndex].click();
                await page.waitForTimeout(400);
                ...
                // Type new value
                await page.keyboard.type(payrollValues[i]);
                await page.keyboard.press('Enter');
                await page.waitForTimeout(300);

                console.log(`✓ Updated Payroll cell ${i + 1}: ${payrollValues[i]}`);
            }
        }

        console.log('Step 7: Clicking Apply for Payroll...');

        // Click Apply button for Payroll row
        await payrollRow.locator('button:has-text("Apply")').first().click();
        await page.waitForTimeout(2000);
        await page.waitForLoadState('networkidle');
        console.log('✓ Applied Payroll changes');
        ...
        const screenshotPath = path.join(process.cwd(), `error_forecast_${Date.now()}.png`);
        try {
