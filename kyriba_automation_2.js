...
async function kyribaAutomation() {
    ...
    try {
        ...
        console.log('Step 4: Setting date range (March 1 - March 31)...');
        ...
        // Click March 31st - last row, last column (Sunday)
        await page.locator('table tbody tr:nth-child(5) td:nth-child(7) button').click();
        console.log('✓ Selected March 31st');

        await page.waitForTimeout(1500);

        console.log('Step 5: Changing Step to Week...');

        // Click on Step dropdown
        await page.click('body > div.flex.min-h-screen.w-full.flex-col.bg-background > div > main > main > div > div > div.border-b > div.px-6.pb-4.flex.items-end.gap-4.text-xs > div:nth-child(8) > button');
        console.log('✓ Clicked Step dropdown');

        await page.waitForTimeout(800);

        // Click on "Week" option - try multiple selectors
        try {
            // Try clicking Week by text within the dropdown
            await page.locator('button:has-text("Week"), div:has-text("Week")').first().click({ timeout: 3000 });
            console.log('✓ Selected Week (method 1)');
        } catch (e) {
            // Fallback: click within the dropdown container
            await page.locator('body > div:nth-child(14)').locator('text=Week').click();
            console.log('✓ Selected Week (method 2)');
        }

        // Wait for the data to reload with weekly view
        await page.waitForTimeout(2000);
        await page.waitForLoadState('networkidle');

        console.log('✓ Date range set: March 1 - March 31, 2024 (Weekly view)');
        ...
        const fileName = 'march_1st_31st_weekly_data.pdf';
